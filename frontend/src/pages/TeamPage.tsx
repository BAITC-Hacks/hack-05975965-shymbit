...
export function TeamPage() {
  ...
  return <div className="page"><div className="container">
    ...
    <header className="team-profile-header"><span className="team-avatar team-avatar--large">{team.name.slice(0, 2).toUpperCase()}</span><div><span className="eyebrow">Студенческая команда</span><h1>{team.name}</h1><Rating rating={team.rating} /></div>{(demo || (session && team.ownerId === session.user.id)) && <Link className="button button--secondary" to={`/teams/${id}/edit`}><Edit3 size={17} />Редактировать профиль</Link>}</header>
    <div className="profile-layout"><div className="profile-main">
      ...
      {(demo || session?.user.role === 'business') && <section className="hub-panel"><h2>Поделитесь опытом работы</h2><p className="muted">Отзыв может оставить организация после принятия отклика этой команды на задачу.</p>
