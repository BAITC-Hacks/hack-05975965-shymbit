...
export function createApp({ store, aiService, allowedOrigins = [], production = false, importLimits, aiRequestLimit = 10, aiConcurrency = 2, trustProxy = false }) {
  const app = express();
  ...
  app.post("/api/tasks/:id/clarify", asyncHandler(async (request, response) => {
    ...
    response.set('ETag', entityTag(updated)).json({ questions: updated.clarificationQuestions, task: updated });
  }));
